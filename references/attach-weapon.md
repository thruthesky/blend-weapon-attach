...
## 목차
0. [v2 — 현재 알고리즘(먼저 읽을 것)](#0-v2--현재-알고리즘)
1. [핵심 개념](#1-핵심-개념)
2. [핵심 로직](#2-핵심-로직)
3. [핵심 소스코드](#3-핵심-소스코드)
...
> 🛑 **§2.1 · §2.2 · §2.3 · §3 · §5 는 v1(구방식) 기록이다.** 개념(원칙 ①·④)과 §4·§6·§7·§8 은
> 그대로 유효하지만, *손잡이 판정·정렬·크기 조절* 은 아래 v2 로 대체됐다. 2026-08-13 남녀 PC
> 17종을 붙이며 v1 이 실제로 깨지는 것을 확인해 바꿨다.

### v1 이 깨진 지점(실측)
| 문제 | v1 동작 | 실측 |
|---|---|---|
| 장축을 local X/Y/Z 중 최대로 가정 | 비스듬한 무기에서 완전히 틀림 | male_vector 검이 local XY 에서 **44.9도** 기울어 길이 98.08 로 측정(실제 138.61, **41% 과소**), 단면 프로파일 무의미 |
| 손잡이를 원형도(round ratio)로 판정 | 둥근 칼날·fuller 에 속음 | codexian 검 전 구간 round>0.7, quantum 칼날 fuller round 0.81 → **칼날을 손잡이로** 오검출 |
| grip 중심을 손 본 head 에 평행이동 | *방향* 을 못 정함 | 검이 옆으로 눕거나 뒤를 향해 `--rot` 로 매번 눈대중 보정 |
| `--ref-height` 로 크기 자동 보정 | 무기 프로파일 json 이 없으면 못 씀 | `game-assets/weapons/*.json` 이 대부분 없음 |

### v2 알고리즘
1. **장축 = PCA.** 정점 공분산의 최대 고유벡터. 무기가 로컬 공간에서 어떻게 놓여 있든 정확하다.
2. **손잡이 = 끝단 ~ 가드.** 장축을 32구간으로 나눠 **가장 넓은 단면 = 가드**로 보고, 가드
   바깥쪽 끝단까지를 손잡이로 잡는다. 그 중심이 주먹에 온다. 가드는 어떤 검이든 최대폭이라 안정적.
3. **파지 프레임 이식.** 이미 잘 붙은 캐릭터에서 뽑은 세 값
   (`grip_H` 손잡이 중심 위치 · `blade_dir_H` 날 방향 · `thin_dir_H` 두께축)을
   **손 본 rest 좌표계** 기준으로 표현해 두고, 대상 무기의 같은 세 값을 여기에 정렬한다
   (회전 = 두 직교정규 기저의 곱, `det=1` 로 거울 반전 없음 확인).
   → 위치뿐 아니라 **방향까지** 맞고, 여러 캐릭터가 같은 자세가 된다.
   프레임이 손 본 로컬이라 **키·체형·아마추어가 달라져도 이식된다**(실측: 16종에서
   `grip_to_fist` 0.031~0.033 으로 일정, 아마추어 교체 후에도 동일).
4. **크기는 기본 유지.** 아티스트 스케일을 그대로 두고 위치·방향만 고친다(`--ratio` 로만 변경).
   비균일 스케일은 **shape space**(스케일 비율만 남긴 좌표계)에서 계산해 **종횡비 보존**.
5. **자동 검증.** 부착 직후 수치 + follow-test + 렌더 2컷을 스크립트가 스스로 수행한다.

### v2 판정 기준 (v1 과 다름)
- v1: `grip_to_hand < 0.02` (grip 중심을 손목에 *붙였으므로* 0 이 나옴)
- v2: **`grip_to_fist` < 0.05** — grip 중심을 손목이 아니라 *파지 프레임이 지정한 위치* 에 두므로
  0 이 아니다. 주먹 중심(손 본 head~tail 중점) 기준으로 재는 것이 의미 있다.
  추가로 `det=1.0000`, `tip_to_wrist ≫ grip_to_wrist`, `####FOLLOW_OK` 를 함께 본다.

### 파일 구성
| 파일 | 역할 |
|---|---|
| `scripts/weapon_geom.py` | PCA 장축 · 가드 기준 손잡이 · shape space. **부착/추출이 공유** |
| `scripts/attach_weapon.py` | 부착 + 자동 검증. 인자 없이 동작 |
| `scripts/extract_frame.py` | 잘 맞춰진 캐릭터에서 파지 프레임 추출(왼손·다른 자세용) |

🛑 **`weapon_geom.analyze()` 를 한쪽만 고치지 말 것.** 참조 추출과 대상 부착이 *다른* 손잡이
정의를 쓰면 "참조의 grip" 과 "대상의 grip" 이 다른 부위를 가리켜 파지가 조용히 어긋난다.
...
> ⚠️ **§2.1~§2.3 은 v1 기록** — 현재 구현은 [§0 v2](#0-v2--현재-알고리즘)를 따른다.
> 아래는 "왜 그렇게 하면 안 되는지" 의 근거로 남겨둔다.

### 2.1 손잡이(grip) 정점 식별 — 원형도 기반 〔v1·대체됨〕
...
### 2.2 grip-align 〔v1·대체됨 → §0 파지 프레임 이식〕
